...
from slowfast.models.losses import KLDiv
...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, _, semantic_prior, affordance_prior, location_prior, meta) in enumerate(train_loader):
        ...
        if 'Prob' in cfg.MODEL.MODEL_NAME:
            preds,atten_map = model(inputs_list, training = True)
            kl = new_criterion(atten_map,location_prior)
        ...
        if 'Prob' in cfg.MODEL.MODEL_NAME or 'Soft' in cfg.MODEL.MODEL_NAME or 'Base' in cfg.MODEL.MODEL_NAME:
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, (inputs, labels, _, semantic_prior, affordance_prior, location_prior, meta) in enumerate(val_loader):
        if cfg.NUM_GPUS:
...
def calculate_and_update_precise_bn(loader, model, cfg, num_iters=200, use_gpu=True):
    ...
    def _gen_loader():
        for inputs, _, _, semantic_prior, affordance_prior, location_prior, _ in loader:
            if use_gpu:
                if isinstance(inputs, (list,)):
                    for i in range(len(inputs)):
                        inputs[i] = inputs[i].cuda(non_blocking=True)
